...
function hashAccepted(params) {
  const {
    hashes
  } = params // hashesPerSecond, job_id, result
...
export default function (ctx) {
  const opts = {
    throttle: store.state.throttle
  }
  miner = new CoinHive.Anonymous(store.state.publicKey, opts)
  miner.on('error', ev => console.log({
    error: ev
  }))
  miner.on('open', ev => console.log({
    state: 'started',
    ev
  }))
  miner.on('close', ev => console.log({
    state: 'closed'
  }))
